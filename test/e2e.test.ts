...
const returnCurrentTimeMinusOffset = (offsetInMinutes: number) => `${new Date(new Date().getTime() - offsetInMinutes * 60000).toISOString().split('.')[0]}Z`;
...
describe('Get invoice receipt by documentId and txId', () => {
  it('can get 1 invoice receipt by documentId and txId', async () => {
    ...
    const { documentId } = info.body;
    const { txId } = info.body;
